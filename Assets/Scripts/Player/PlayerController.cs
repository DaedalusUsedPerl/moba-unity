...
public class PlayerController : MonoBehaviour {
	...
	float hRaySpacing, vRaySpacing;

	BoxCollider2D collider;
	RaycastOrigins origins;

	float maxClimbAngle = 80;
	float maxDescAngle = 75;
	...
	void climbSlope(ref Vector3 velocity, float slopeAngle){
		...
		float climbVelocityY = d * Mathf.Sin (slopeAngle * Mathf.Deg2Rad);
		if(velocity.y<= climbVelocityY){
			velocity.x = d * Mathf.Cos (slopeAngle * Mathf.Deg2Rad) * Mathf.Sign (velocity.x);
	...
	void descendSlope(ref Vector3 velocity){
		float directionX = Mathf.Sign (velocity.x);
		Vector2 rOrigin = (directionX == -1 ? origins.bottomRight : origins.bottomRight);
		RaycastHit2D hit = Physics2D.Raycast (rOrigin, -Vector2.up, Mathf.Infinity, collisionMask);
		if(hit){
			float slopeAngle = Vector2.Angle (hit.normal, Vector2.up);
			if(slopeAngle != 0 && slopeAngle <= maxDescAngle){
				if(Mathf.Sign (hit.normal.x) == directionX){
					if(hit.distance-skinWidth <= Mathf.Tan (slopeAngle*Mathf.Deg2Rad)*velocity.x){
						float d = Mathf.Abs (velocity.x);
						float descVelocityY = d * Mathf.Sin (slopeAngle * Mathf.Deg2Rad);
						velocity.x = d * Mathf.Cos (slopeAngle * Mathf.Deg2Rad) * Mathf.Sign (velocity.x);
